...
"""
    try:
        ...
        logging.error(f"Erro ao enviar e-mail de confirmação: {erro}")
        return False

def enviar_pendencia(destinatario, documentos_faltantes):
    assunto = "Pendência na Documentação"
    lista_documentos = "\n".join(f"- {doc}" for doc in documentos_faltantes)
    corpo = f"""Olá,
...
"""
    try:
        ...
        logging.info(f"E-mail de pendência enviado para {destinatario}")
        return True
    except Exception as erro:
        logging.error(f"Erro ao enviar e-mail de pendência: {erro}")
        return False